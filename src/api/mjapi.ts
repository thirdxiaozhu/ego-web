...
export const myFetch = (url: string, data?: any) => {
  // mlog('myFetch', url  );
  const header = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${getToken()}` }
  // header= {...header  }
...
export const canVisionModel = (model: string) => {
  ...
  if (['gpt-4-all', 'gpt-4-v', 'gpt-4v', 'gpt-3.5-net'].includes(model))
    return true
  if (model.includes('gpt-4-gizmo') || model.includes('claude-3-opus'))
    return true
